...
///// ################################################ 
RcppExport SEXP CAAKepler_Calculate(SEXP M_, SEXP e_, SEXP nIterations_)
{ 
  double M = as<double>(M_);
  double e = as<double>(e_);
  int nIterations = as<int>(nIterations_); 
  ...
  //Convert the result back to degrees
  return wrap(CAACoordinateTransformation::RadiansToDegrees(E) * F);
}

...
RcppExport SEXP CAAJewishCalendar_DateOfPesach(SEXP Year_, SEXP bGregorianCalendar_)
{ 
  ...
  if (Pesach.Day > 31)
  {
    ...
    Pesach.Year = A;
  return (List::create(Named("Year") = Pesach.Year, Named("Month") = Pesach.Month, Named("Day") =Pesach.Day));
}

RcppExport SEXP CAAJewishCalendar_IsLeap(SEXP Year_)
{
   long Year = as<long>(Year_);
  long ymod19 = Year % 19;

  return wrap((ymod19 == 0) || (ymod19 == 3) || (ymod19 == 6) || (ymod19 == 8) || (ymod19 == 11) || (ymod19 == 14) || (ymod19 == 17));
}

RcppExport SEXP CAAJewishCalendar_DaysInYear(SEXP Year_)
{ 
  long Year = as<long>(Year_);
  //Find the previous civil year corresponding to the specified jewish year
  long CivilYear = Year - 3761;
  ...
  //Find the date of the next Jewish Year in that civil year
  CAACalendarDate CurrentPesach = CAAJewishCalendar::DateOfPesach(CivilYear);
  bool bGregorian = CAADate::AfterPapalReform(CivilYear, CurrentPesach.Month, CurrentPesach.Day);
  CAADate CurrentYear(CivilYear, CurrentPesach.Month, CurrentPesach.Day, bGregorian);

  CAACalendarDate NextPesach = CAAJewishCalendar::DateOfPesach(CivilYear+1);
  CAADate NextYear(CivilYear+1, NextPesach.Month, NextPesach.Day, bGregorian);

  return wrap(static_cast<long>(NextYear - CurrentYear));
}

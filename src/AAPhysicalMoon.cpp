...
RcppExport SEXP CAAPhysicalMoon_CalculateGeocentric(SEXP JD_)
{
  ...
  double epsilon;
  CAA2DCoordinate Equatorial;
  CAAPhysicalMoonDetails details = CAAPhysicalMoon::CalculateHelper(JD, Lambda, Beta, epsilon, Equatorial);
  List res = List::create(Named("P") = details.P, Named("b") = details.b, Named("l") = details.l);
  return res; 
}

RcppExport SEXP CAAPhysicalMoon_CalculateTopocentric(SEXP JD_, SEXP Longitude_, SEXP Latitude_)
{ 
  double JD = as<double>(JD_); 
...
RcppExport SEXP CAAPhysicalMoon_CalculateSelenographicPositionOfSun(SEXP JD_)
{ 
  double JD = as<double>(JD_); 
  double R = CAAEarth::RadiusVector(JD)*149597970;
  double Delta = CAAMoon::RadiusVector(JD);
  ...
  double bdash20;
  double epsilon;
  CAAPhysicalMoon::CalculateOpticalLibration(JD, lambdah, betah, ldash0, bdash0, ldash20, bdash20, epsilon, omega, DeltaU, sigma, I, rho);

  details.l0 = ldash0 + ldash20;
  details.b0 = bdash0 + bdash20;
  details.c0 = CAACoordinateTransformation::MapTo0To360Range(450 - details.l0);
  List res = List::create(Named("l0") = details.l0, Named("b0") = details.b0, Named("c0") = details.c0);
  return res;
}

RcppExport SEXP CAAPhysicalMoon_AltitudeOfSun(SEXP JD_, SEXP Longitude_, SEXP Latitude_)
{ 
  double JD = as<double>(JD_);
  double Longitude = as<double>(Longitude_);
  double Latitude = as<double>(Latitude_);
  //Calculate the selenographic details
  CAASelenographicMoonDetails selenographicDetails = CAAPhysicalMoon::CalculateSelenographicPositionOfSun(JD);
  ...
  return wrap(CAACoordinateTransformation::RadiansToDegrees(asin(sin(selenographicDetails.b0)*sin(Latitude) + cos(selenographicDetails.b0)*cos(Latitude)*sin(selenographicDetails.c0 + Longitude))));
}


RcppExport SEXP CAAPhysicalMoon_TimeOfSunrise(SEXP JD_, SEXP Longitude_, SEXP Latitude_)
{
  double JD = as<double>(JD_);
  double Longitude = as<double>(Longitude_);
  double Latitude = as<double>(Latitude_);
  return wrap(CAAPhysicalMoon::SunriseSunsetHelper(JD, Longitude, Latitude, true));
}

RcppExport SEXP CAAPhysicalMoon_TimeOfSunset(SEXP JD_, SEXP Longitude_, SEXP Latitude_)
{ 
  double JD = as<double>(JD_);
  double Longitude = as<double>(Longitude_);
  double Latitude = as<double>(Latitude_);
  return wrap(CAAPhysicalMoon::SunriseSunsetHelper(JD, Longitude, Latitude, false));
}

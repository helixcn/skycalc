...
RcppExport SEXP CAAParabolic_Calculate(SEXP JD_, SEXP elements_JDEquinox_, SEXP elements_omega_, 
                                       SEXP elements_w_, SEXP elements_i_, SEXP elements_T_, 
                                       SEXP elements_q_ )
{  
  ...
  for (int j=0; j<2; j++)
  {
    double W = 0.03649116245/(elements_q * sqrt(elements_q)) * (JD0 - elements_T);
    double s = CAAParabolic::CalculateBarkers(W);
    double v = 2*atan(s);
    double r = elements_q * (1 + s*s);
    double x = r * a * sin(A + w + v);
    double y = r * b * sin(B + w + v);
    double z = r * c * sin(C + w + v);
  ...
  return List::create(Named("HeliocentricRectangularEquatorial.X") = details.HeliocentricRectangularEquatorial.X  ,
                       ...
                       Named("AstrometricGeocenticRA") = details.AstrometricGeocenticRA               ,
                       ...
                       Named("PhaseAngle") = details.PhaseAngle                           );
}

...
RcppExport SEXP CAADate_AfterPapalReform_Date(SEXP Year_, SEXP Month_, SEXP Day_)
{ 
  long Year   = as<long>(Year_);
  long Month  = as<long>(Month_);
  double Day  = as<double>(Day_);
  
  return wrap((Year > 1582) || ((Year == 1582) && (Month > 10)) || ((Year == 1582) && (Month == 10) && (Day >= 15)));
}

RcppExport SEXP CAADate_AfterPapalReform_JD(SEXP JD_)
{
  double JD = as<double>(JD_); 
  return wrap((JD >= 2299160.5));
}

RcppExport SEXP CAADate_DateToJD(SEXP Year_, SEXP Month_, SEXP Day_, SEXP bGregorianCalendar_)
{
  long Year   = as<long>(Year_ );
  long Month  = as<long>(Month_);
  double Day  = as<double>(Day_);
  bool bGregorianCalendar = as<bool>(bGregorianCalendar_);
  ...
  if (bGregorianCalendar)
  {
    long A = CAADate::INT(Y / 100.0);
    B = 2 - A + CAADate::INT(A / 4.0);
  }

  return (wrap(CAADate::INT(365.25 * (Y + 4716)) + CAADate::INT(30.6001 * (M + 1)) + Day + B - 1524.5));
}

RcppExport SEXP CAADate_IsLeap(SEXP Year_, SEXP bGregorianCalendar_)
{ 
   long Year = as<long>(Year_); 
   bool bGregorianCalendar = as<bool>(bGregorianCalendar_); 
  ...
  {                                       
    if ((Year % 100) == 0)                
      return wrap(((Year % 400) == 0) ? true : false);
    else
      return wrap(((Year % 4) == 0) ? true : false);
  }
  else                                        
    return wrap(((Year % 4) == 0) ? true : false);
}
...
RcppExport SEXP CAADate_DaysInMonth(SEXP Month_, SEXP bLeap_)
{ 
  long Month = as<long>(Month_);
  bool bLeap = as<bool>(bLeap_);
  ...
  return wrap(MonthLength[Month-1]);
}



RcppExport SEXP CAADate_DayOfYear(SEXP JD_, SEXP Year_, SEXP bGregorianCalendar_)
{
  double JD               = as<double>(JD_);
  long Year               = as<long>(Year_);
  bool bGregorianCalendar = as<bool>(bGregorianCalendar_);
  
  return wrap(JD - CAADate::DateToJD(Year, 1, 1, bGregorianCalendar) + 1);
}

RcppExport SEXP CAADate_INT(SEXP value_)
{
  double value   = as<double>(value_ );
  if (value >= 0)
    return wrap(static_cast<long>(value));
  else
    return wrap(static_cast<long>(value - 1));
}

RcppExport SEXP CAADate_JulianToGregorian(SEXP Year_, SEXP Month_, SEXP Day_)
{ 
  long Year   = as<long>(Year_ );
  long Month  = as<long>(Month_);
  long Day    = as<long>(Day_  );
  ...
  return (List::create(Named("Year") = GregorianDate.Year,Named("Month") = GregorianDate.Month,Named("Day") = GregorianDate.Day, Named("Hour") = Hour, Named("Minute") = Minute, Named("Second") = Second));  
}

RcppExport SEXP CAADate_GregorianToJulian(SEXP Year_, SEXP Month_, SEXP Day_)
{
  long Year   = as<long>(Year_ );
  long Month  = as<long>(Month_);
  long Day    = as<long>(Day_  );
  ...
  date.Get(JulianDate.Year, JulianDate.Month, JulianDate.Day, Hour, Minute, Second);
   ...
   return (List::create(Named("Year") = JulianDate.Year,Named("Month") = JulianDate.Month,Named("Day") = JulianDate.Day, Named("Hour") = Hour, Named("Minute") = Minute, Named("Second") = Second));   
}

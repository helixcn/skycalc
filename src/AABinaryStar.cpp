...
RcppExport SEXP CAABinaryStar_Calculate(SEXP t_, SEXP P_, SEXP T_, SEXP e_, SEXP a_, SEXP i_, SEXP omega_, SEXP w_)
{ 
  ...
  return List::create(Named("r") = details.r, Named("Theta") = details.Theta, Named("Rho") = details.Rho);
}

RcppExport SEXP CAABinaryStar_ApparentEccentricity(SEXP e_, SEXP i_, SEXP w_)
{ 
  double e  = as<double>(e_);
  double i  = as<double>(i_);
  double w  = as<double>(w_);
  ...
  double sqrtD = sqrt(D);
  return wrap(sqrt(2*sqrtD / (A + C + sqrtD)));
}

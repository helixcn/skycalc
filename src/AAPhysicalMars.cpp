...
RcppExport SEXP CAAPhysicalMars_Calculate(SEXP JD_)
{
  ...
  List res = List::create(Named("DE") = details.DE,Named("DS") = details.DS,Named("w") = details.w, Named("P") =details.P ,Named("X") =details.X ,Named("d") = details.d,Named("k") = details.k,Named("q") = details.q); 
  return res;
}

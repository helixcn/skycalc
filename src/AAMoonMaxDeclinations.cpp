...
RcppExport SEXP  CAAMoonMaxDeclinations_K(SEXP Year_)
{
  double Year = as<double>(Year_); 
  return wrap(13.3686*(Year - 2000.03));
}

RcppExport SEXP  CAAMoonMaxDeclinations_MeanGreatestDeclination(SEXP k_, SEXP bNortherly_)
{ 
  double k = as<double>(k_);
  bool bNortherly  = as<bool>(bNortherly_);
  ...
  double value = bNortherly ? 2451562.5897 : 2451548.9289;
  return wrap(value + 27.321582247*k + 0.000119804*T2 - 0.000000141*T3);
}

RcppExport SEXP  CAAMoonMaxDeclinations_MeanGreatestDeclinationValue(SEXP k_)
{
  double k = as<double>(k_);
  //convert from K to T
  double T = k/1336.86;
  return wrap(23.6961 - 0.013004*T);
}

RcppExport SEXP  CAAMoonMaxDeclinations_TrueGreatestDeclination(SEXP k_, SEXP bNortherly_)
{
  double k = as<double>(k_);
  bool bNortherly  = as<bool>(bNortherly_);
  //convert from K to T
  ...
  return wrap(CAAMoonMaxDeclinations::MeanGreatestDeclination(k, bNortherly) + DeltaJD);
}

RcppExport SEXP  CAAMoonMaxDeclinations_TrueGreatestDeclinationValue(SEXP k_, SEXP bNortherly_)
{
  double k = as<double>(k_);
  bool bNortherly  = as<bool>(bNortherly_);
  //convert from K to T
  ...
  return wrap(CAAMoonMaxDeclinations::MeanGreatestDeclinationValue(k) + DeltaValue);
}

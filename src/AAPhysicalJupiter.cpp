...
RcppExport SEXP CAAPhysicalJupiter_Calculate(SEXP JD_)
{ 
  ...
  //Step 18
  details.P = CAACoordinateTransformation::MapTo0To360Range(CAACoordinateTransformation::RadiansToDegrees(atan2(cos(delta0dashrad)*sin(alpha0dashrad - alphadashrad), sin(delta0dashrad)*cos(deltadashrad) - cos(delta0dashrad)*sin(deltadashrad)*cos(alpha0dashrad - alphadashrad))));
  List res = List::create(Named("DS") =details.DS ,Named("DE") = details.DE,Named("Geometricw1") =details.Geometricw1 ,Named("Geometricw2") =details.Geometricw2 ,Named("Apparentw1") =details.Apparentw1 ,Named("Apparentw2") = details.Apparentw2,Named("P") = details.P);
  return res;
}

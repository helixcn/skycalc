...
RcppExport SEXP CAAParallax_DistanceToParallax(SEXP Distance_)
{ 
  double Distance = as<double>(Distance_); 
  double pi = asin(g_AAParallax_C1 / Distance);
  return wrap(CAACoordinateTransformation::RadiansToDegrees(pi));
}

RcppExport SEXP CAAParallax_ParallaxToDistance(SEXP Parallax_)
{ 
  double Parallax = as<double>( Parallax_); 
  return  wrap( g_AAParallax_C1 / sin(CAACoordinateTransformation::DegreesToRadians(Parallax)));
}

RcppExport SEXP CAAParallax_Equatorial2TopocentricDelta(SEXP Alpha_, SEXP Delta_, SEXP Distance_, SEXP Longitude_, SEXP Latitude_, SEXP Height_, SEXP JD_)
{ 
  ...
  DeltaTopocentric.X = CAACoordinateTransformation::RadiansToHours(-pi*RhoCosThetaPrime*sinH/cosDelta);
  DeltaTopocentric.Y = CAACoordinateTransformation::RadiansToDegrees(-pi*(RhoSinThetaPrime*cosDelta - RhoCosThetaPrime*cosH*sin(Delta)));
  return List::create(Named("DeltaTopocentric.X") = DeltaTopocentric.X, Named("DeltaTopocentric.Y") = DeltaTopocentric.Y);
}

RcppExport SEXP CAAParallax_Equatorial2Topocentric(SEXP Alpha_, SEXP Delta_, SEXP Distance_, SEXP Longitude_, SEXP Latitude_, SEXP Height_, SEXP JD_)
{ 
  double Alpha = as<double>( Alpha_);
  double Delta = as<double>( Delta_);
  double Distance = as<double>( Distance_);
  ...
  return List::create(Named("Topocentric.X") = Topocentric.X, Named("Topocentric.Y") = Topocentric.Y);
}

RcppExport SEXP CAAParallax_Ecliptic2Topocentric(SEXP Lambda_, SEXP  Beta_, SEXP  Semidiameter_, SEXP  Distance_, SEXP Epsilon_, SEXP Latitude_, SEXP Height_,SEXP JD_)
{
  double  Lambda       = as<double>( Lambda_       );
  double  Beta         = as<double>( Beta_         );
  double  Semidiameter = as<double>( Semidiameter_ );
  double  Distance     = as<double>( Distance_     );
  ...
  return  List::create(Named("Topocentric.Semidiameter") = Topocentric.Semidiameter, Named("Topocentric.Lambda") = Topocentric.Lambda, Named("Topocentric.Beta") =Topocentric.Beta);
}

...
///// ###########################################
///// ###########################################
///// Not consisted with the results from SKYCALC by Espanek and Occult. 

RcppExport SEXP CAAMoonPerigeeApogee_K(SEXP Year_)
{
  double Year = as<double>(Year_);
  return wrap(13.2555*(Year - 1999.97));
}

RcppExport SEXP CAAMoonPerigeeApogee_MeanPerigee(SEXP k_)
{  
  ...
  return wrap(2451534.6698 + 27.55454989*k - 0.0006691*Tsquared - 0.000001098*Tcubed + 0.0000000052*T4);
}

RcppExport SEXP CAAMoonPerigeeApogee_MeanApogee(SEXP k_)
{  
  double k = as<double>(k_);
  //Uses the same formula as MeanPerigee
  return wrap(CAAMoonPerigeeApogee::MeanPerigee(k));
}

RcppExport SEXP CAAMoonPerigeeApogee_TruePerigee(SEXP k_)
{ 
  double k = as<double>(k_);
  double MeanJD = CAAMoonPerigeeApogee::MeanPerigee(k);
  ...
  return wrap(MeanJD + Sigma);
}

RcppExport SEXP CAAMoonPerigeeApogee_PerigeeParallax(SEXP k_)
{ 
  double k = as<double>(k_);
  //convert from K to T
  double T = k/1325.55;
  ...
  return wrap(Parallax/3600);
}

RcppExport SEXP CAAMoonPerigeeApogee_TrueApogee(SEXP k_)
{ 
  double k = as<double>(k_);
  double MeanJD = CAAMoonPerigeeApogee::MeanApogee(k);
...
RcppExport SEXP CAAMoonPerigeeApogee_ApogeeParallax(SEXP k_)
{
  double k = as<double>(k_);
  //convert from K to T
  double T = k/1325.55;

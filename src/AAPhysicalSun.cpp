...
RcppExport SEXP CAAPhysicalSun_Calculate(SEXP JD_)
{ 
  ...
  double eta = atan(tan(SunLong - K)*cos(I));
  details.L0 = CAACoordinateTransformation::MapTo0To360Range(CAACoordinateTransformation::RadiansToDegrees(eta - theta));
  List res = List::create(Named("P") = details.P, Named("B0") = details.B0, Named("L0") = details.L0);
  return res;
}

RcppExport SEXP CAAPhysicalSun_TimeOfStartOfRotation(SEXP C_)
{ 
  long C = as<long>(C_); 
  double JED = 2398140.2270 + 27.2752316*C;

...
//// ###################################################################
//// ###################################################################
//// Not Accurate after implementation. Use with care. 

RcppExport SEXP CAAMoslemCalendar_MoslemToJulian(SEXP Year_, SEXP Month_, SEXP Day_)
{ 
  ...
  JulianDate.Year = X;
  CAADate::DayOfYearToDayAndMonth(J, CAADate::IsLeap(X, false), JulianDate.Day, JulianDate.Month);
   // long DayOfYear, bool bLeap, long& DayOfMonth, long& Month
  return List::create(Named("JulianDate.Year") = JulianDate.Year, 
                       Named("JulianDate.Month") = JulianDate.Month,
                       Named("JulianDate.Day") = JulianDate.Day 
                       );
}

RcppExport SEXP CAAMoslemCalendar_JulianToMoslem(SEXP Year_, SEXP Month_, SEXP Day_)
{
  long Year  = as<long>(Year_);
  long Month = as<long>(Month_);
  long Day   = as<long>(Day_);

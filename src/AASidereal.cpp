...
RcppExport SEXP CAASidereal_MeanGreenwichSiderealTime(SEXP JD_)
{
  ...
  return (wrap(CAACoordinateTransformation::MapTo0To24Range(Value)));
}

RcppExport SEXP CAASidereal_ApparentGreenwichSiderealTime(SEXP JD_)
{
  double JD = as<double>(JD_); 
  double MeanObliquity = CAANutation::MeanObliquityOfEcliptic(JD);
  double TrueObliquity = MeanObliquity + CAANutation::NutationInObliquity(JD) / 3600;
  ...
  double Value = CAASidereal::MeanGreenwichSiderealTime(JD) + (NutationInLongitude  * cos(CAACoordinateTransformation::DegreesToRadians(TrueObliquity)) / 54000);
  return (wrap(CAACoordinateTransformation::MapTo0To24Range(Value)));
}

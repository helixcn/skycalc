...
RcppExport SEXP CAAIlluminatedFraction_PhaseAngle_1(SEXP r_, SEXP R_, SEXP Delta_)
{ 
  ...
  //Return the result
  return wrap(CAACoordinateTransformation::MapTo0To360Range(CAACoordinateTransformation::RadiansToDegrees(acos((r*r + Delta*Delta - R*R) / (2*r*Delta)))));
}

RcppExport SEXP CAAIlluminatedFraction_PhaseAngle_2(SEXP R_, SEXP R0_, SEXP B_, SEXP L_, SEXP L0_, SEXP Delta_)
{ 
  double R     = as<double>(R_    );
  double R0    = as<double>(R0_   );
  double B     = as<double>(B_    );
  double L     = as<double>(L_    );
  double L0    = as<double>(L0_   );
  double Delta = as<double>(Delta_);
  ...
  //Return the result
  return wrap(CAACoordinateTransformation::MapTo0To360Range(CAACoordinateTransformation::RadiansToDegrees(acos((R - R0*cos(B)*cos(L - L0))/Delta))));
}

RcppExport SEXP CAAIlluminatedFraction_PhaseAngleRectangular(SEXP x_, SEXP y_, SEXP z_, SEXP B_, SEXP L_, SEXP Delta_)
{ 
 double x     = as<double>(x_    ); 
 double y     = as<double>(y_    ); 
 double z     = as<double>(z_    ); 
 double B     = as<double>(B_    ); 
 double L     = as<double>(L_    ); 
 double Delta = as<double>(Delta_);
  ...
  //Return the result
  return wrap(CAACoordinateTransformation::MapTo0To360Range(CAACoordinateTransformation::RadiansToDegrees(acos( (x*cosB*cos(L) + y*cosB*sin(L) + z*sin(B)) / Delta) )));
}

RcppExport SEXP CAAIlluminatedFraction_IlluminatedFraction_1(SEXP PhaseAngle_)
{ 
...
RcppExport SEXP CAAIlluminatedFraction_IlluminatedFraction_2(SEXP r_, SEXP R_, SEXP Delta_)
{
  double r     = as<double>(r_    );
  double R     = as<double>(R_    );
  double Delta = as<double>(Delta_);
  return wrap(((r+Delta)*(r+Delta) - R*R) / (4*r*Delta));
}

RcppExport SEXP CAAIlluminatedFraction_MercuryMagnitudeMuller(SEXP r_, SEXP Delta_, SEXP i_)
{ 
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double i         = as<double>(i_    );
  double I_50      = i - 50;
  return wrap(1.16 + 5*log10(r*Delta) + 0.02838*I_50 + 0.0001023*I_50*I_50);
}

RcppExport SEXP CAAIlluminatedFraction_VenusMagnitudeMuller(SEXP r_, SEXP Delta_, SEXP i_)
{ 
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double i         = as<double>(i_    );
  return wrap(-4.00 + 5*log10(r*Delta) + 0.01322*i + 0.0000004247*i*i*i);
}

RcppExport SEXP CAAIlluminatedFraction_MarsMagnitudeMuller(SEXP r_, SEXP Delta_, SEXP i_)
{ 
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double i         = as<double>(i_    );
  return wrap(-1.3 + 5*log10(r*Delta) + 0.01486*i);
}

RcppExport SEXP CAAIlluminatedFraction_JupiterMagnitudeMuller(SEXP r_, SEXP Delta_)
{ 
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  return wrap(-8.93 + 5*log10(r*Delta));
}

RcppExport SEXP CAAIlluminatedFraction_SaturnMagnitudeMuller(SEXP r_, SEXP Delta_, SEXP DeltaU_, SEXP B_)
{ 
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double DeltaU     = as<double>(DeltaU_);
  double B     = as<double>(B_);
  //Convert from degrees to radians
  B = CAACoordinateTransformation::DegreesToRadians(B);
  double sinB = sin(B);

  return wrap(-8.68 + 5*log10(r*Delta) + 0.044*fabs(DeltaU) - 2.60*sin(fabs(B)) + 1.25*sinB*sinB); 
}

RcppExport SEXP CAAIlluminatedFraction_UranusMagnitudeMuller(SEXP r_, SEXP Delta_)
{ 
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  return wrap(-6.85 + 5*log10(r*Delta));
}

RcppExport SEXP CAAIlluminatedFraction_NeptuneMagnitudeMuller(SEXP r_, SEXP Delta_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  return wrap(-7.05 + 5*log10(r*Delta));
}

RcppExport SEXP CAAIlluminatedFraction_MercuryMagnitudeAA(SEXP r_, SEXP Delta_, SEXP i_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double i         = as<double>(i_    );
  ...
  return wrap(-0.42 + 5*log10(r*Delta) + 0.0380*i - 0.000273*i2 + 0.000002*i3);
}

RcppExport SEXP CAAIlluminatedFraction_VenusMagnitudeAA(SEXP r_, SEXP Delta_, SEXP i_)
{ 
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double i         = as<double>(i_    );
  ...
  return wrap(-4.40 + 5*log10(r*Delta) + 0.0009*i + 0.000239*i2 - 0.00000065*i3);
}

RcppExport SEXP CAAIlluminatedFraction_MarsMagnitudeAA(SEXP r_, SEXP Delta_, SEXP i_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double i         = as<double>(i_    );
  return wrap(-1.52 + 5*log10(r*Delta) + 0.016*i);
}

RcppExport SEXP CAAIlluminatedFraction_JupiterMagnitudeAA(SEXP r_, SEXP Delta_, SEXP i_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double i         = as<double>(i_    );
  return wrap(-9.40 + 5*log10(r*Delta) + 0.005*i);
}

RcppExport SEXP CAAIlluminatedFraction_SaturnMagnitudeAA(SEXP r_, SEXP Delta_, SEXP DeltaU_, SEXP B_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  double DeltaU     = as<double>(DeltaU_);
  ...
  return wrap(-8.88 + 5*log10(r*Delta) + 0.044*fabs(DeltaU) - 2.60*sin(fabs(B)) + 1.25*sinB*sinB); 
}

RcppExport SEXP CAAIlluminatedFraction_UranusMagnitudeAA(SEXP r_, SEXP Delta_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  return wrap(-7.19 + 5*log10(r*Delta));
}

RcppExport SEXP CAAIlluminatedFraction_NeptuneMagnitudeAA(SEXP r_, SEXP Delta_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  return wrap(-6.87 + 5*log10(r*Delta));
}

RcppExport SEXP CAAIlluminatedFraction_PlutoMagnitudeAA(SEXP r_, SEXP Delta_)
{
  double r         = as<double>(r_    );
  double Delta     = as<double>(Delta_);
  return wrap(-1.00 + 5*log10(r*Delta));
}

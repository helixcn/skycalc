...
RcppExport SEXP CAAEclipses_CalculateSolar(SEXP k_)
{
double k = as<double>(k_);
  ...
  double Mdash = 0;
  CAASolarEclipseDetails res;
  res = CAAEclipses::Calculate(k, Mdash);
  return (List::create(Named("F") = res.F,
                        Named("TimeOfMaximumEclipse") = res.TimeOfMaximumEclipse,
                        Named("gamma") = res.gamma,
                        Named("u") = res.u,
                        Named("bEclipse") = res.bEclipse,
                        Named("GreatestMagnitude") = res.GreatestMagnitude)); 
}


RcppExport SEXP CAAEclipses_CalculateLunar(SEXP k_)
{
double k = as<double>(k_);
#ifdef _DEBUG
  double intp = 0;
  ...
  double Mdash = 0;
  CAASolarEclipseDetails solarDetails = CAAEclipses::Calculate(k, Mdash);
  ...
  return List::create(Named("bEclipse") = details.bEclipse                                                 ,
                       Named("F") = details.F                                                               ,
                       Named("gamma") = details.gamma                                                       ,
                       Named("TimeOfMaximumEclipse") = details.TimeOfMaximumEclipse                         ,
                       Named("u") = details.u                                                               ,
                       Named("PenumbralRadii") = details.PenumbralRadii                                     ,
                       Named("UmbralRadii") = details.UmbralRadii                                           ,
                       Named("PenumbralMagnitude") = details.PenumbralMagnitude                             ,
                       Named("UmbralMagnitude") = details.UmbralMagnitude                                   ,
                       Named("PartialPhaseSemiDuration") = details.PartialPhaseSemiDuration                 ,
                       Named("TotalPhaseSemiDuration") = details.TotalPhaseSemiDuration                     ,
                       Named("PartialPhasePenumbraSemiDuration") = details.PartialPhasePenumbraSemiDuration );
}

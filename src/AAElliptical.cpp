...
double CAAElliptical::LengthOfEllipse(double e, double a)
{
  ...
  return 3.1415926535897932384626433832795 * (3 * (a+b) - sqrt((a+3*b)*(3*a + b)));
}
...
{ 
  ...
  return wrap(Distance * 0.0057755183);
}

//// SUN
RcppExport SEXP CAAElliptical_Calculate_Sun(SEXP JD_){
    ...
    return(List::create(   Named("ApparentGeocentricDistance")=details.ApparentGeocentricDistance,
                           ...
                           Named("ApparentGeocentricRA")=details.ApparentGeocentricRA,
                           Named("ApparentGeocentricDeclination")=details.ApparentGeocentricDeclination
                          ));
}

////Mercury
RcppExport SEXP CAAElliptical_Calculate_Mercury(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
////Venus
RcppExport SEXP CAAElliptical_Calculate_Venus(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
////Mars
RcppExport SEXP CAAElliptical_Calculate_Mars(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
////JUPITER
RcppExport SEXP CAAElliptical_Calculate_Jupiter(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
////SATURN
RcppExport SEXP CAAElliptical_Calculate_Saturn(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
////URANUS
RcppExport SEXP CAAElliptical_Calculate_Uranus(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
////NEPTUNE
RcppExport SEXP CAAElliptical_Calculate_Neptune(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
////PLUTO
RcppExport SEXP CAAElliptical_Calculate_Pluto(SEXP JD_){
    double JD = as<double>(JD_);
    CAAEllipticalPlanetaryDetails details;
...
RcppExport SEXP CAAElliptical_SemiMajorAxisFromPerihelionDistance(SEXP q_, SEXP e_)
{ 
  double q = as<double>(q_);
  double e = as<double>(e_);
  return wrap(q / (1 - e));
}

RcppExport SEXP CAAElliptical_MeanMotionFromSemiMajorAxis(SEXP a_)
{ 
  double a = as<double>(a_);
  return wrap(0.9856076686 / (a * sqrt(a)));
}

//// # .Call("CAAElliptical_Calculate", 2451501, 2.2091404, 0.8502196, 11.94524, 186.23352, 334.75006, 2451544.5, 2448192.5)

RcppExport SEXP CAAElliptical_Calculate(SEXP JD_, 
     SEXP elements_a_        ,
     SEXP elements_e_        ,
     SEXP elements_i_        ,
     SEXP elements_w_        ,
     SEXP elements_omega_    ,
     SEXP elements_JDEquinox_,
     SEXP elements_T_
)
{ double JD = as<double>(JD_); 
  ...
  double Epsilon = CAANutation::MeanObliquityOfEcliptic(elements_JDEquinox);
  ...
  Epsilon = CAACoordinateTransformation::DegreesToRadians(Epsilon);
  double omega = CAACoordinateTransformation::DegreesToRadians(elements_omega);
  double w = CAACoordinateTransformation::DegreesToRadians(elements_w);
  double i = CAACoordinateTransformation::DegreesToRadians(elements_i);
  ...
  CAA3DCoordinate SunCoord = CAASun::EquatorialRectangularCoordinatesAnyEquinox(JD, elements_JDEquinox);

  for (int j=0; j<2; j++)
  {
    double M = n * (JD0 - elements_T);
    double E = CAAKepler::Calculate(M, elements_e);
    E = CAACoordinateTransformation::DegreesToRadians(E);
    double v = 2*atan(sqrt((1 + elements_e) / (1 - elements_e)) * tan(E/2));
    double r = elements_a * (1 - elements_e*cos(E));
    double x = r * a * sin(A + w + v);
    double y = r * b * sin(B + w + v);
    double z = r * c * sin(C + w + v);
